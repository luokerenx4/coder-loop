...
> 更新于 2026-06-12（第 5 版）。本文件的读者是**调度者**——负责按下方波次表启动任务区块、观察完成、推进下一波的 agent。波次表是操作员已裁决的执行顺序，**不是建议**：不得提前、不得重排、不得合并或拆分轨道。遇到本文件未覆盖的情况，停下报告操作员，不要自行变通。
...
```mermaid
flowchart LR
  W1["W1（当前）<br/>3 条轨道"] --> W2["W2<br/>5 条轨道"] --> W3["W3<br/>4 条轨道"] --> W4["W4<br/>4 条轨道"] --> W5["W5<br/>#407"] --> W6["W6<br/>#409→#410"] --> W7["W7<br/>#419"]
```
