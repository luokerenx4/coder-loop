...
Blocker durably posted → write state per `/Users/mouriya/Ext/app/coder-loop/presets/gh-issue-pr-iteration/review/actions/state-write.md` with transition `blocked` (including `--blocker-repo` / `--blocker-ref`). Publication failed → do not write local state as if the blocker were durable; take the stop action.
