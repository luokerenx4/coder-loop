...
5. **Title subject re-check**. Re-read the title with the laziest-PR simulation. Does the simulated PR title's subject match the issue title's subject? If the simulated PR could rename the subject and still pass, the title-intent acceptance hole exists. Sharpen title or add subject-binding row.
