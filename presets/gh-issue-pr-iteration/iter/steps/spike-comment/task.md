...
3. Gather evidence: run the commands in `## 验证步骤` / `## 验收标准`; read referenced PRs/commits/issues/docs. Capture outputs under `EVIDENCE_DIR` so the comment can cite them. Evidence rules: section A of `/Users/mouriya/Ext/app/coder-loop/presets/gh-issue-pr-iteration/quality/evidence.md`.
