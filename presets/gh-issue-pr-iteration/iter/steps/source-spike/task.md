...
3. Run the strongest feasible runtime checks (evidence rules: section A of `/Users/mouriya/Ext/app/coder-loop/presets/gh-issue-pr-iteration/quality/evidence.md`). When browser evidence is required and the spike has browser-observable behavior, capture it; otherwise record why it is not applicable plus the runtime proof that is.
